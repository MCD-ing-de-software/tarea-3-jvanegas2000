...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        """
        # Listo el escenario esperado en el docstring, ahora implemento el test siguiendo esos pasos
        df = make_sample_df()
        cleaner = DataCleaner()
        # Act
        result = cleaner.drop_invalid_rows(df, ["name", "age"])
        # Assert - sin valores faltantes en las columnas especificadas
        self.assertEqual(result["name"].isna().sum(), 0)
        self.assertEqual(result["age"].isna().sum(), 0)
        # Assert - menos filas que el original
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        """
        # Arreglo el escenario esperado en el docstring, ahora implemento el test siguiendo esos pasos
        df = make_sample_df()
        cleaner = DataCleaner()
        # Act & Assert
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        """
        # dataframe con espacios en blanco ya lo tenemos en make_sample_df(), ahora implemento el test siguiendo los pasos del escenario esperado
        df = make_sample_df()
        cleaner = DataCleaner()
        # Act
        result = cleaner.trim_strings(df, ["name"])
        # Assert - el original NO fue modificado
        self.assertEqual(df["name"].iloc[0], " Alice ")
        self.assertEqual(df["name"].iloc[3], " Carol  ")
        # Assert - el resultado tiene los espacios eliminados
        self.assertEqual(result["name"].iloc[0], "Alice")
        self.assertEqual(result["name"].iloc[3], "Carol")
        # Assert - columnas no especificadas no cambian
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        """
        # Dataframe con columnas numéricas ya lo tenemos en make_sample_df(), ahora implemento el test siguiendo los pasos del escenario esperado
        df = make_sample_df()
        cleaner = DataCleaner()
        # Act & Assert
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        - Verificar que se lanza un TypeError (usar self.assertRaises)
        """
        # Dataframe con columnas de texto ya lo tenemos en make_sample_df(), ahora implemento el test siguiendo los pasos del escenario esperado
        df = make_sample_df()
        cleaner = DataCleaner()
        # Act & Assert
        with self.assertRaises(TypeError):
