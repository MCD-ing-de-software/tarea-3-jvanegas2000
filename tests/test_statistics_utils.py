...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Assert - valores correctos con tolerancia flotante
        expected = np.array([1.5, 2.5, 3.5])
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        # Assert - forma (shape) esperada
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        utils = StatisticsUtils()
        arr = [1, 2, 3]
        # Act & Assert - window=0 debe lanzar ValueError
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        # Act & Assert - window mayor que el array debe lanzar ValueError
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        utils = StatisticsUtils()
        arr_2d = [[1, 2], [3, 4]]
        # Act & Assert - secuencia 2D debe lanzar ValueError
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        # Assert - media aproximadamente 0
        self.assertAlmostEqual(float(result.mean()), 0.0, places=10)
        # Assert - desviacion estandar aproximadamente 1
        self.assertAlmostEqual(float(result.std()), 1.0, places=10)
